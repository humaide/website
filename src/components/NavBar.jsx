...
function NavBar() {
  ...
  return (
    <nav className="flex flex-col bg-pastel-blue/25 backdrop-blur-xl text-charcoal w-full font-sans fixed z-100">
      {/* mobile nav icon */}
      <div className={`flex lg:hidden self-end justify-between items-center w-full px-4 py-2 z-40`}>
        <div className="flex items-center">
          <img src="logos/logo-dark-accent.svg" alt="Humaide Logo" className={`h-10 w-10 ${isOpen ? "hidden" : ""}`} />
          <img src="logos/logo-light-accent.svg" alt="Humaide Logo" className={`h-10 w-10 ${isOpen ? "" : "hidden"}`} />
        </div>
        <button
          onClick={() => setIsOpen(true)}
          className={`${isOpen ? "hidden" : ""} flex items-center px-3 py-2 rounded text-black-500 hover:text-black-400`}
        >
          <FontAwesomeIcon icon="bars" size="2xl" className="text-charcoal hover:cursor-pointer" />
        </button>
      </div>

      {/* nav content */}
      <div
        className={`w-full z-50 bg-charcoal lg:bg-pastel-blue/25 backdrop-blur-xl self-end h-full lg:h-fit absolute lg:relative flex flex-col lg:flex lg:justify-between items-center text-white sm:text-charcoal grow xl:text-lg 2xl:text-xl 3xl:text-3xl 4xl:text-5xl ${isOpen ? "flex" : "hidden"}`}
      >
        <div className="w-full flex flex-col lg:flex-row lg:justify-between text-white sm:text-charcoal items-center bg-charcoal sm:bg-pastel-blue/25 backdrop-blur-xl">
          ...
          {/* menu items */}
          <div className="lg:w-[60%] sm:bg-pastel-blue lg:bg-transparent flex flex-col lg:flex-row lg:justify-evenly text-white sm:text-charcoal items-center w-full">
