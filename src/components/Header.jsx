...
function Header({ subtitle, title, description, position, color }) {
    return (
      <>
      <motion.div transition={{ duration: 0.5, ease: "backIn"}} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} lang='en'  className={`flex flex-col items-center justify-center w-full md:w-[55%] max-md:px-4 max-lg:px-0 lg:px-28 2xl:px-44 py-2 max-lg:text-center mb-12 ${color == "green" ? "bg-midnight-blue" : "bg-gray-100 lg:bg-transparent"} ${position == "right" ? "lg:text-left lg:items-start " : position == "left" ? "lg:items-end lg:text-right" : "lg:items-center lg:text-center"}`}>
